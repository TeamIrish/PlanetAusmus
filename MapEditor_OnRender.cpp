...
void MapEditor::OnRender() {
  ...
  /* render player character and HUD elements (or grave if game over) */
	// player is alive
	if(playerHealth > 0) {
	  ...
	  if(dispMenu == true && Running) Surface::OnDraw(Surf_Display,Menu,(WWIDTH-MENU_W)/2,(WHEIGHT-MENU_H)/2);
	}

	// player is dead
	else{
	  Surface::OnDraw(Surf_Display,Grave,(WWIDTH-CHARACTER_W)/2,(WHEIGHT-CHARACTER_H)/2);
	  Surface::OnDraw(Surf_Display,GameOverText,WWIDTH/2-225,WHEIGHT/2-50);
	}
